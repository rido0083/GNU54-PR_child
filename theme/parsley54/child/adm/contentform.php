<?php
// include_once (G5_ADMIN_PATH.'/pr_adm/admin.head.php');
...
$html_title = "내용";
$g5['title'] = $html_title.' 관리 (PR-CHILD)';
...
if ($w == "u")
{
    ...
    // pr-child
    $sql_pr = " select * from ".PR_CONTENT_EXP." where co_id = '$co_id' ";
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <!-- pr-child -->
    <tr>
        <th scope="row"><label for="pr_co_filename">CHILD-FILE</label></th>
        <td>
          <select name="pr_co_filename" id="pr_co_filename">
          ...
          foreach ($files as $f) {
              $page_name = explode('.',$f);
          ?>
            <option value="<?php echo $page_name[0];?>" <?php echo ($pr_co_value['co_filename'] == trim($page_name[0])) ? 'selected' : ''?>>
              <?php echo $page_name[0]; ?>
            </option>
          <?php
          }
          ?>
    ...
    <tr>
        <th scope="row"><label for="pr_access_mblv">권한설정</label></th>
        <td>
          <?php echo get_member_level_select('pr_access_mblv', 1, $member['mb_level'], $pr_co_value['access_mblv'] ) ?>
          * 해당컨텐츠 열람시 필요한 회원권한을 설정합니다.
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="pr_co_point">차감포인트</label></th>
        <td>
          <input type="number" name="pr_co_point" value="<?php echo $pr_co_value['co_point']?>" id="pr_co_point" class="frm_input" size="20">
          * 해당컨텐츠 열람시 필요한 포인트를 설정합니다.
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="pr_co_view">내용표시</label></th>
        <td>
          <select name="pr_co_view" id="pr_co_view">
            <option value="Y" <?php echo ($pr_co_value['co_view'] == 'Y') ? 'selected' : ''?>> 내용출럭 </option>
            <option value="N" <?php echo ($pr_co_value['co_view'] == 'N') ? 'selected' : ''?>> 내용미출력 </option>
          </select>
          * 해당컨텐츠 열람시 아래 입력한 내용을 표시 여부입니다.
          표시할 경우 해당 페이지의 상단에 출력이 됩니다.
        </td>
    </tr>
    <!-- pr-child -->
